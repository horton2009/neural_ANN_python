...
Classes for parsing and generating AnnML XML files."""

__author__       = 'Sam Gibson'
__version__      = '0.1'
__lastmodified__ = 'Wed Feb  9 00:49:03 PST 2005'
...
class XMLBPNSaver :
   ...
   def _writeneuron ( self, neuron, outfile ) :
      ...
      if ( neural.InputNeuron == type( neuron ) ) :
...
class XMLNeuralSaver :
   ...
   def save ( self, network, xmlfile ) :
      ...
      if ( neural.BackPropNet == type( network ) ) :
...
class XMLNeuralHandler (ContentHandler) :			# Realize the metod of startElemnt( ) and endElement( ) : basic for ContentHandler
   ...
   def endElement ( self, name ) :
      if ( 'neuron' == name ) :
         if ( neural.InputLayer != type( self._currentlayer ) ) :
   ...
   def _createNet ( self, type ) :
      ...
      if ( 'bpn' == type ) :
         self._network = neural.BackPropNet()
         self._nettype = 'bpn'
      else :
   ...
   def _addLayer ( self, type, neurons ) :		 # add empty neurons
      ...
      if ( 'input' == type ) :
         self._currentlayer = self._inputlayer = neural.InputLayer( int( neurons ) )
      elif ( 'hidden' == type ) :
         self._currentlayer = neural.HiddenLayer( int( neurons ) )
         self._hiddenlayers.append( self._currentlayer )
      elif ( 'output' == type ) :
         self._currentlayer = self._outputlayer = neural.OutputLayer( int( neurons ) )
      else :
